...
export class DraggableState {
	...
	/** Requested position by mouse, pixels, set while active to immediately move to this position */
	requestedPosition = new DOMPoint(0, 0)
	/** Requested rotation, degrees (0 = no rotation), set to animate to a specific rotation */
	...
	/**
	 * Init plays an initial animation for the draggable element and adds itself to the simulation
	...
	init(index: number) {
		...
		const anim = this.el.animate(
			[{ transform: this.config.initialTransform }, { transform: this.transform() }],
			{
	...
	/** Destroy removes the draggable element from the simulation and resets the element's styles. */
	destroy() {
	...
	/**
	 * Fling applies a "fling" effect to the draggable element, giving it a random velocity and
	 * angular velocity.
	 */
	fling() {
	...
	onMouseDown(ev: MouseEvent) {
		...
		this.el.style.zIndex = (lastZ++).toString()
...
/**
 * ExtractTransformCoordinates extracts the 2D transform coordinates from a DOM element. It falls
 * back to parsing the transform string if CSSStyleValue is not supported. If parsing fails as well,
 * it returns null.
 */
...
/**
 * ParseTransform attempts to parse a CSS transform string and extract the 2D translation
 * coordinates, using any of the following formats: - translate(x, y) - translate3d(x, y, z) -
 * translateX(x) translateY(y) - translateY(y) translateX(x)
 *
