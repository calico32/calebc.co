...
/**
 * A SimulationConfig holds the physics and animation properties for the card
 * simulation.
 */
...
export class Simulation {
  ...
  /** new creates a new DraggableState for the given HTMLElement bound to this
   * Simulation. */
  new(el: HTMLElement) {
  ...
  /** zIndex returns an incrementing z-index for the draggable elements, to
   * ensure cards that are interacted with are displayed on top. */
  zIndex(): number {
    this.#z++
    return this.#z
  }
  ...
  clamp(value: number, min: number, max: number): number {
    if (value < min) {
      return (
        min -
        Math.tanh((min - value) / this.config.movementClamp) *
          this.config.movementClamp
      )
    }
    if (value > max) {
      return (
        max +
        Math.tanh((value - max) / this.config.movementClamp) *
          this.config.movementClamp
      )
    }
    return value
  ...
  /** push1d applies a "push" effect to a value, moving it closer to a target
   * range. */
  ...
  /** push2d applies a "push" effect to a 2D point, moving it closer to a target
   * region. */
